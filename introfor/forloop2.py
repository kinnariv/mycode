...
farms = [{"name": "NE Farm", "agriculture": ["sheep", "cows", "pigs", "chickens", "llamas", "cats"]},
         {"name": "W Farm", "agriculture": ["pigs", "chickens", "llamas"]},
         {"name": "SE Farm", "agriculture": ["chickens", "carrots", "celery"]}]# create a second list of strings
approved_vendors = ["cisco", "juniper", "big_ip"]
# loop across the list called vendors
for x in Farm:
    print("\nThe Farm is " + x, end="")   # newline, print current Farm, and end without newline
    if x not in approved_Farms:   # if x does not appear within the list Farm
        print(" - NOT IN LIST FARM!", end="")
print("\nOur loop has ended.") # print when loop has finished
#!/usr/bin/env python3
"""RZFeeser | Alta3 Research
   nesting an if-statement inside of a for loop"""
